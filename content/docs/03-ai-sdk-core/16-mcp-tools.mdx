...
the OAuth authorization server to use. Metadata discovery rejects
private, link-local, and non-HTTP(S) authorization server URLs by default and
does not follow HTTP redirects. If you connect to MCP servers outside your
control, also implement `validateAuthorizationServerURL` on your
